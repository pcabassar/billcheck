...
- It is **removed from the active pnpm workspace** (so installs/build/CI only see the greenfield app
  in `apps/billcheck/`). The internal `workspace:*` links here no longer resolve — it's not meant to run.
...
Layout: `apps/web` (the click-through shell), `packages/engine` (the deterministic audit engine + D10
