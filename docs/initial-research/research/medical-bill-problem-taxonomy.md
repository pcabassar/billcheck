# US Medical-Bill Problem Taxonomy — research pass for billcheck

> Problem-space research for billcheck — US, patient/consumer side. · _2026-06-16_

**Date:** 2026-06-16 · **Purpose:** map the landscape of medical-bill problems a US
patient can face, to fuel the brainstorm. **Scope:** US only, patient/consumer
...
1. **There are recognizable "situations" with known plays — but treat this as a helpful map, not a
   closed set.** A patient is *often* in one (or more) of ~20 recognizable situations, each with a
   *signal*, a *confirming artifact*, a *lever*, a *legal hook*, and a *deadline*. These map well onto
   an orchestrator-picks-tools architecture (§8).
...
deadlines. Strong candidate for a "tool."
...
## 8. Strategic implications for billcheck (orchestrator + tools)
...
3. **What's conversation/judgment:** triage to the right situation, deciding which
   ...
   where chat/voice earns its keep — and where a rigid linear flow would fall short.
4. **Re-rank the roadmap by consumer leverage, not by "coolest check."** Highest
   ...
   **C3 GFE/PPDR**, **D1 FDCPA validation**. Classic coding-error checks (A3/A4/A5)
   ...
   keep them, but as *leverage*, not the headline. **Note:** the data on denials→appeal
   (huge volume, tiny appeal rate, ~50%+ win) argues for making it a first-class,
   agent-assisted flow.
...
6. **Treat legal facts as live config, not constants.** Credit-reporting rules (§6),
   ...
   2026. The knowledge base needs dated, versioned facts (a versioned-reference discipline).
