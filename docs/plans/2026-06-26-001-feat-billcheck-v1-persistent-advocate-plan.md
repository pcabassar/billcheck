...
### Next build (v1.01 — ready to pick up, not "someday")
- **Cited retrieval/lookup tool (R13)** — a model-callable tool that fetches *current, cited* coverage/rules facts the model + user docs can't supply (a specific ACA plan's coverage; a law's current status) from grounded public data (CMS marketplace + Transparency-in-Coverage files; a dated rules library). **The socket already exists in U4's tool surface — additive, not a re-architecture.**
- **Branding/naming pass** — name + bot persona + voice + welcome copy; decouple-able (a config string), runs parallel to the build.
- **Share-card structured-field whitelist (U7)** — add only if prompt-first cards show real PII leaks.
...
- **Review the profile-field structure (U8 bookmark)** — are the open `situationNotes` accreting facts that should be promoted to structured fields? Revisit **after v1.01 ships**.
...
- **Prompt caching for documents; compress the *conversation*, never trim documents.** Cache the inlined docs and keep them ALL in context — the window is 1M and bills are small, so doc-trimming solves a non-problem. The lever for a long-running case is conversation compaction: **`prepareStep` + `pruneMessages`** (drop stale tool-call/reasoning blocks) layered under the derived summary + structured-state seed. (Evaluate Anthropic's provider-side `contextManagement` `compact_*` — zero app code — once Gateway pass-through is confirmed.) There is no native SDK summarizer; the derived summary is the compaction backbone.
...
- **Conclusion = a maintained case status + a transition-triggered share check + an always-available button.** The model emits a typed **case status every turn** (`new → gathering → recommendation-offered → acting → resolved`) via `output: Output.object` — reliable *because* it's every turn, not a one-shot flag. On a **status transition** to a milestone, a focused `generateObject` check (full context) decides *is this share-worthy, and drafts the share content*, surfacing a tailored offer. The always-available "wrap up / share" button (→ `generateShareCard`) is the backstop. `markResolved`/`reopenCase` set the closed status the reminder reads. No `flagConclusion` one-shot tool.
...
- **The aggregate-consent guard is server-enforced:** the aggregate write checks consent **before computing** the record (server-side, not the model's call). The **share card starts prompt-generated** (the model drafts an anonymized card) with the **user previewing it before they share** — the human preview is the v1 PII backstop (the user catches their own name/amount). A structured-field whitelist is **deferred** until we observe real leaks. *(Simplicity-first: don't build the guard until we learn we need it.)*
...
- **Lean on the SDK's free features (verified v6.0.209):** assemble the 3-part prompt + inject per-step summary/state via **`prepareStep`**; emit the per-turn status via **`output: Output.object`** (the canonical name — `experimental_output` is deprecated; budget **+1 `stopWhen` step** since structured output counts as a step); a separate **`generateObject`** for the on-transition share check; persist via server **`onFinish({ messages })` + `createIdGenerator()` + `validateUIMessages` on load**; read **`usage.inputTokenDetails.cacheReadTokens`** in `onFinish` to *prove* caching hits (if 0 on a repeat turn, fall back from the Gateway string to the direct `@ai-sdk/anthropic` provider); plus per-message `metadata` (model/cost provenance), `maxRetries`, `abortSignal`, and `activeTools` (canonical, not `experimental_`).
...
## Output Structure
    ...
    lib/
      ...
      share/card.ts               # prompt-generated anonymized card
...
**Approach:**
- Route: `requireUserId()` → resolve/create the active case → **deterministically persist the incoming turn** → load **summary + structured state** (not raw transcripts) → inline only **owner-verified** blobs (resolved from `documents` by `(caseId, userId)`, never the client `part.url` — the authorization boundary; surface fetch failures) → assemble the 3-part prompt + inject summary/state via **`prepareStep`** → apply `cacheControl` to the system prefix + the last doc part → `streamText(..., tools, stopWhen, output: Output.object(status))` → **persist the turn deterministically** in the server **`onFinish({ messages })`** (`createIdGenerator()` for stable IDs; `validateUIMessages` on load).
...
- Keep the doc set/order **stable** turn-to-turn so the cache holds. **No document trimming** (1M window, small docs). Compact a long *conversation* with `prepareStep` + `pruneMessages` (drop stale tool-call/reasoning blocks) under the summary seed — the derived summary is the compaction backbone.
...
**Files:** Create `lib/share/card.ts` (anonymized card builder), `app/(app)/cases/[id]/` wrap-up/share UI; Modify `lib/tools/{generate-share-card,mark-resolved,reopen-case}.ts`, `app/page.tsx`; Test `test/share-card-pii.test.ts`

**Approach:** an **always-available "wrap up / share" affordance** calls `generateShareCard` (agent-callable AND the button's backstop — same function), the card is **prompt-generated** (the model drafts an anonymized version) and the **user previews it before sharing** — the human preview is the v1 PII backstop; a structured-field whitelist is deferred until we see leaks. One card per case (regenerated, not duplicated); honest copy for non-win outcomes. The model emits a typed **case status every turn** via `output: Output.object` (`new → gathering → recommendation-offered → acting → resolved`); a server-detected **transition to a milestone** fires a focused `generateObject` check (full context) that decides *is this share-worthy* and drafts the card content, surfacing a tailored offer inline — nothing gates on it. `markResolved` (user or agent) sets `caseStatus='resolved'` (what the reminder reads); `reopenCase` is its inverse (re-opens, supersedes the card). No `flagConclusion` one-shot tool.

**Test scenarios:** model never signals → the button still produces a valid card; the previewed card reads as anonymized (manual check; whitelist hardening deferred until leaks observed); a status transition to `recommendation-offered` fires the share-worthy check and surfaces a tailored offer but doesn't close anything; `markResolved` closes + the reminder suppresses; `reopenCase` re-opens + supersedes the card.
...
**Approach:** start **minimal + open** — the cross-case research is blunt that *insurance situation is the master key* (it gates which levers are even legal), so structure that one thing well and leave the rest soft. **Structured fields:** `coverageSituation` (the routing key — precise enough to pick the playbook: uninsured / commercial-in-network / commercial-OON / Medicare-FFS / Medicare-Advantage / Medicaid / dual-QMB / ACA-marketplace / two-plan-COB), two flags that flip the whole playbook (**dual/QMB** → $0; **self-funded vs fully-insured** → state-DOI vs ERISA venue), and **state**. Everything else (veteran, income, charity-eligibility, Medicaid-pending, specific plan/employer name) lives in an **open free-text `situationNotes`** the model fills as it learns — *not* pre-built columns; promote a fact to its own field only once it recurs and drives a decision (reviewed for v1.1 — see Scope). `updateProfile` (model-proposed, server-validated, **field-level merge**) persists facts as revealed; the profile is injected each turn as its **own labeled block** (distinct from the case summary) so the model never re-probes; the UI shows an editable panel; deterministic protections key off the structured flags (QMB → $0); artifacts auto-fill. The user's **plan documents** (e.g. a commercial SBC) attach at the **profile level** so they persist across cases. *Rules vs. specifics: the model handles the **rules framework** (QMB, NSA limits, ACA §2713, Medicare ladders) well at a point in time; **instance/current specifics** (a specific plan's coverage, a law's current status) come from the user + their docs in v1, unknowable externals left as attributed placeholders — with a cited **retrieval/lookup tool as the top v1.01 add** (R13; socket in U4).*

**Test scenarios:** "QMB" stored in session 1 → applied in session 3 without re-asking; the profile renders as an editable panel and edits persist; a profile fact fills an artifact blank; two concurrent profile edits merge (no lost update); a non-routing fact (e.g. veteran) lands in `situationNotes`, not a new column; invalid diff rejected.
...
**Approach:** case list + a **no-cases home + warm welcome/greeting** that, in plain language, reassures, says what billcheck does, and invites the first action (relief, not homework — final voice/copy from the brand pass); auto-create the first case on first message; case detail (timeline, artifacts, wrap-up/share); render `tool-*` parts as **plain-language ✓ steps with collapsed details** (never raw JSON); approval cards (edit-before-approve) for the gated tools; artifact download + share-card UI; consent UI; **upload-failure fallback ("just tell me about it")**; re-auth-without-loss; a single-active-session ("opened elsewhere") notice. Reuse the white-space + remark-gfm fixes on every new markdown surface; run a browser/integration pass.
...
| Long-conversation context growth | Low | Med | Conversation compaction (`prepareStep`+`pruneMessages`) + derived summary; no doc-trimming (1M window + cached small docs) |
| Prompt caching not engaging via the Gateway | Med | Low | Verify `usage.inputTokenDetails.cacheReadTokens` in `onFinish`; fall back to direct `@ai-sdk/anthropic` if 0 |
...
- **Document context:** prompt caching (keep all docs in context, no doc-trimming); long conversations compacted via `prepareStep`+`pruneMessages` under the derived summary (no native SDK summarizer exists).
- **Conclusion/share:** a per-turn `output: Output.object` case status + an on-transition `generateObject` share-worthy/draft check + an always-available `generateShareCard` button; `markResolved`/`reopenCase` are explicit status actions; no `flagConclusion` trigger.
